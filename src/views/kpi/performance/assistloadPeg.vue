...
<script>
  ...
  import { getAllResult, changeCols, getColumns, sortByNum, refreshAllResult, getCubePegAssist } from '@/api/kpi/performance/workloadPeg'
  export default {
    name: '',
    components: {
    ...
    mounted() {
      ...
      if (this.type === 'assistant') { // 办案工作-法官助理-工作量反查页面
      ...
      this.params = this.fd.tools.merge(true, {}, this.tableParams, this.pParams.params)
      this.userName = this.selfMsg.userName || this.$store.getters.user.userName
      // this.getColumns(this.type) // 获取工作量反查的表头数据
      if (this.from) {
        this.getCubePegAssist()
      } else {
        this.refreshAllResult()
      }
    },
    methods: {
      getAllResult, searchByKeyword, sortByNum, refreshAllResult, getCubePegAssist,
      currentChange, sizeChange, downloadTable, getColumns, changeCols, goBack
    },
    watch: {}
  }
</script>
