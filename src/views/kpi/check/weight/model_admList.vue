          ...
          <div>
            <fd-table
              ...
              <el-table-column
                ...
                label="操作">
                <template slot-scope="scope">
                  <el-button type="text" v-show="elements.edit && edit" @click="editModelDetail(scope.row)">编辑</el-button>
                  <el-button type="text" v-show="elements.delete && edit" @click="deleteModelList(scope.row)">删除</el-button>
                </template>
              </el-table-column>
            </fd-table>
          </div>
    ...
    >
      <div slot="fd-body">
        <el-form :rules="rules"
          ...
          <el-form-item label="指标规则" prop="ruleId" v-if="formData.typeId !== 'k10021-02' && formData.typeId !== 'k10021-02'">
          ...
          <el-form-item label="" prop="formulaId" v-show="formData.typeId !== 'k10021-02' && formData.ruleId == 'k10025-02'">
            <el-select v-model="formData.formulaId"
                        filterable
                        multiple
                        :disabled="isShow"
                        placeholder="请选择行政管理范围">
              <el-option
                v-for="item in admType"
                :key="item.id"
                :label="item.name"
                :value="item.id">
              </el-option>
      ...
      <div slot="fd-footer">
        ...
        <el-button v-show="!isShow" size="small" type="primary" @click="getData = !getData">确定</el-button>
      </div>
    </fd-dialog>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    data() {
      ...
      const validateFormulaId = (rule, value, callback) => {
        if (this.formData.ruleId === 'k10025-02' && value.length === 0) {
          callback(new Error('请选择行政管理范围'))
        } else {
          callback()
        }
      }
      return {
        ...
        rules: {
          ...
          formulaId: [{ validator: validateFormulaId, trigger: 'blur' }],
    ...
    computed: {
      ...
      // 行政管理范围
      admType() {
        return this.$store.getters.selects['04023']
      },
      admTypeName() {
        const types = this.$store.getters.selects['04023'] || []
        const typeNames = {}
        types.forEach(d => {
          typeNames[d.id] = d.name
        })
        return typeNames
      },
      // 指标类型
