    ...
    <div class="bg-white container-inner has-fixed-bottom clearfix h14 check-list" :style="{'min-height':height}">
      <div class="bd-dashed-bottom pb2 mb2">
        <div class="clearfix mb2 height50 line-height-50">
          <div class="f-left color-title bold h20">{{schema.departName || '--'}}</div>
        </div>
        <div>
          <el-row class="mb2">
            <el-col :span="8">
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <span v-for="(items, index) of scope.row.scores" v-if="getTargetData(items)" :key="items.id">
...
<script>
  import { getRuleDetailById, refreshTableData, onProgress } from '@/api/kpi/checkResultDepart/mzIndexDepartPeg'
