<template>
    ...
    <multi-head
      ...
      >
        <template slot-scope="scope">
          ...
          <div v-if="info.id==='approvalScore' && info.isGotoDetail === undefined">
            <el-popover
              placement="top"
              width="300"
              trigger="click"
  ...
  <el-table-column
    ...
    <template slot-scope="scope">
      ...
      <div v-else-if="data.field === 'chairName'">{{scope.row.chairName?scope.row.chairName: '--'}}</div>
...
<script>
  import { casePeg, cbrWorkPeg, moralPeg, showPeg, getPegValue, staffNamePeg, sortByNum, mzIndexPeg,
    targetPeg, getAlignStyle } from '@/api/kpi/checkResult/tableDetail'
  export default {
    name: 'multiHead',
    components: {},
    props: {
    ...
    methods: {
      casePeg, cbrWorkPeg, moralPeg, showPeg, getPegValue, staffNamePeg, sortByNum, mzIndexPeg,
      targetPeg, getAlignStyle,
