...
<script>
  ...
  import FdFormulaDesDepart from '../components/mzFormulaDesDepart'
  ...
  export default {
    ...
    methods: {
      ...
      formulaChange(id) {
        if (id === '12' || this.formulaMap[id].value === '自定义公式') {
      ...
      getFormulaList() {
        this.fd.req({
          url: 'api/blocks/formulas?type=democratic_depart',
          method: 'get'
        }).then((d) => {
          const data = d.data
