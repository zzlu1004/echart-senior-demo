    ...
    <el-table class="tree-table"
      ...
      >
        <template slot-scope="scope">
          ...
          <!--指标流程-->
          <span v-if="item.field === 'process' && dataMap[scope.row.id].menuType === '1'">
          ...
          <!--规则-->
          <div class="pointer blue-pointer" @click.stop="showRule(dataMap[scope.row.id])" v-else-if="item.getSum && item.field === 'rule' && dataMap[scope.row.id].menuType === '0'">
            ...
            <span class="f-right edit-icon">
                <i class="fd-icon icon-tianxiecopy"/>
            </span>
          </div>
          <!--分数换算-->
          <div class="pointer blue-pointer" @click.stop="showFormula(dataMap[scope.row.id])" v-else-if="item.getScore && item.field === 'formulas' && dataMap[scope.row.id].menuType === '1'">
          ...
          <!--得分区间-->
          <span class="pointer relative blue-pointer"  @click.stop="showSection(dataMap[scope.row.id])" v-else-if="item.changed && item.field === 'checkFormula' && dataMap[scope.row.id].menuType === '1'">
          <span class="inline-block pr3" v-html="getField(item, scope.row.id)"></span>
        ...
        <div class="dialog-formulaForm">
          <el-form ref="formulaForm" label-position="right" :model="formulaForm" label-width="0" @submit.native.prevent>
            <div v-for="(item, index) in formulaList" class="mt2 relative" :key="index">
              ...
              <fd-formula-des v-show="formulaForm.formulaId === item.id && focusing"></fd-formula-des>
            </div>
        ...
        <div class="dialog-formulaForm">
          <el-form ref="formulaRuleForm" :rules="rules" label-position="right" :model="formulaRuleForm" label-width="0" @submit.native.prevent>
            ...
            <el-form-item label="" prop="formula" v-show="formulaRuleForm.ruleType === 'k10030-02'">
              ...
              <fd-formula-des :des="dess" :example="[]" ></fd-formula-des>
            </el-form-item>
          </el-form>
        </div>
      </div>
            ...
            <el-form-item label-width="0">
              <div>
                ...
                <div class="p2 bd">
                  ...
                  <fd-formula-des :des = "des"></fd-formula-des>
                  <div class="pt2">
                    ...
                    <div class = "except-w6-full">
                      <el-form-item prop="indexSetupValue">
                        ...
                        <el-popover
                          ...
                          width="300"
                          trigger="hover"
                          content="指标分数为非必填项,当该个人指标设置某种限制条件时,该指标数值为该分数。">
...
<script>
  ...
  import FdFormulaDes from '../components/mzFormulaDes'
  ...
  export default {
    ...
    data() {
      ...
      const validateRuleFormula = (rule, value, callback) => {
        ...
        // 验证this.menus所有指标名称menuType和小于层级recentMenuLevel的目录名称
      ...
      注意: 1、输入公式仅使用以下16种的半角符号才有效;
        ...
        focusing: true,
        recentMenuLevel: 0 // 判断当前选择规则的目录的层级，仅可选择当前层级以下的目录作为规则
      }
    },
    computed: {},
    mounted() {
    ...
    methods: {
      ...
      getField(headItem, key) {
        ...
        if (headItem.field === 'indexType' && obj.menuType === '1') {
          ...
          if (obj.flows) {
            return obj.flowName || obj.flows[0].name
          } else {
            return obj.flowName || '--'
          }
      ...
      showFormula(item) {
        ...
        if (this.formulaMap[item.formulaId]) {
      ...
      getFormulaName(item) {
        if (item.id === '4' && this.formulaMap[item.id].value === '自定义公式') {
          return '自定义公式'
        } else {
          return this.formulaMap[item.id].value
        }
      },
      // 分数换算radio选择自定义公式时，默认展示placeholder的内容
      formulaChange(id) {
        if (id === '11' || this.formulaMap[id].value === '自定义公式') {
      ...
      getFormulaList() {
        this.fd.req({
          url: 'api/blocks/formulas?type=democratic_personal',
          method: 'get'
        }).then((d) => {
          const data = d.data
