              ...
              <div v-else>
                  ...
                  <span v-else-if="headItem.field === 'approveScore' && item.approveHasMe">
                    <el-popover
                      ...
                      <span v-else slot="reference" :class="item.allStatus !=='已完成'?'color-red': ''">{{item.draftScore}}</span>
                    </el-popover>
                  </span>
                  <span v-else v-html="getField(headItem, key)"></span>
              </div>
...
<script>
  ...
  import { seeRegularDetail, saveDraft, saveSubmit, seeAttachment, recallTarget,
    ...
    downloadFile, checkApproveScore, beforeAvatarUpload } from '@/api/kpi/onlineCheck/checkList'
    ...
    data() {
      return {
        ...
        tableHeads: [{
          ...
          field: 'attachments',
          label: '材料说明',
          width: this.fd.tools.getStringWidth('材料说明名'),
          default: ''
        }, {
          field: 'operate',
          label: '操作',
