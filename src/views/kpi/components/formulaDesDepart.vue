...
<script>
  ...
  export default {
    name: 'FdFormulaDesDepart',
    components: {},
    props: {
      des: {
        type: String,
        default: `公式示例: 80+(((部门指标值*10)-全院指标均值)/全院指标均值)*100+IF(部门指标值>0 AND
      部门指标值<100,90,0)+IF(全院指标值>80 OR 全院指标值>0,90,0)
      ...
      example: {
        type: Array,
        default: () => {
          return ['部门指标值', '全院指标值', '全院指标均值', '业务线指标值', '业务线指标均值']
        }
      },
      example2: {
        type: Array,
        default: () => {
          return ['+', '-', '*', '/', 'IF', 'AVERAGE', '(', ')', 'AND', 'OR', ',', '>', '<', '=', '>=', '<=']
        }
      }
    },
    data() {
