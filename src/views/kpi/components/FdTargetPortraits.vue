<template>
  <div class="target-drag-list">
    <el-row>
      <el-col :span="11">
        <div class="height-normal line-height-normal mb1">已选指标 ({{menusTreeNum}})</div>
        <div id = "targetLeftMenu" name="toMenuList" class="p1 bd bg auto radius-normal" style="height: 22rem;">
          <fd-target-drag v-bind:data="menusTree" v-on:refreshMenus="refreshDataMap"></fd-target-drag>
          <!--<div @click="addMenuParent"-->
               <!--class="bd-dashed px2 py1 mt2 pointer"-->
               <!--style="margin-left: 45px;">新建分类</div>-->
        </div>
      </el-col>
      <el-col :span="13">
        <div class="pl2">
            <div class="height-normal line-height-normal mb1 pl2">
                指标库
                <el-popover
                placement="bottom-start"
                width="310"
                trigger="hover"
                content="点击指标目录,指标目录下的指标可以拖动到左侧各个分类下面。">
          ...
          <div class="p1 bd-dashed relative target-ku radius-normal">
            <!-- <fd-search className="absolute fd-search" style="width: 180px;top: -40px; right: 0px"
                        @search="searchTarget"
                        v-model="indexName"></fd-search> -->
            ...
            <!-- <div>
              <el-radio-group v-model="indexType" @change="getTargetList" size="small">
                <el-radio-button label="report" border>综合填报({{reportCnt}})</el-radio-button>
                <el-radio-button label="newSystem" border>系统测算({{newSystemAccessCnt}})</el-radio-button>
              </el-radio-group>
            </div> -->
            <div class="auto full" id = "targetRightMenu" style="height: 21rem">
              <div class="px1 mt1" v-for="(item, index) of targetData" :key="index">
                <i v-if="!item.show" @click="showTarget(index, true)" class="at icon-caretright pointer"/>
                <i v-else @click="showTarget(index, false)" class="at icon-caretdown"/>
                <span>{{item.name}}</span>
                <div v-show="item.show">
                  <draggable name="targetList" :options="targetOpt" @clone="clone" @end="end"  @start="start" :move="move">
                    <div :id="child.id" v-bind:class="isDraggable(child)" v-for="child of item.children" :key="child.id">
...
<script>
  import {
    getTargetList,
    showTarget
  } from '@/api/kpi/record/portraits'
  import {
    getTargetCount
  } from '@/api/kpi/check/target'
  import draggable from 'vuedraggable'
  import FdTargetDrag from './targetDragPortaits'
  export default {
    name: 'FdTarget',
    ...
    computed: {
      menusTreeNum() {
        if (this.menusTree.length > 0) {
          let num = 0
          this.menusTree.forEach(v => {
            num += v.targets.length
          })
          return num
        } else {
          return this.menusTree.length
        }
      }
    },
    mounted() {
