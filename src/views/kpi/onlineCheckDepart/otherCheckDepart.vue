    ...
    <div class="bg-white container-inner clearfix h14 target" :style="{'min-height': height }">
      <div class="tab-box">
        <el-tabs v-model="params.type" type="card">
          <el-tab-pane :label="'已填报（'+reportCnt+'）'" name="to_fill">
          ...
          <el-tab-pane :label="'已审核（'+checkCnt+'）'" name="to_approve">
          ...
          <el-tab-pane :label="'民主测评（'+democraticCnt+'）'" name="to_mz">
            <mzCheckingDepart ref="mzCheckingDepart" v-show="params.type ==='to_mz'" checkType='to_mz' :active="params.type" :parentParams="params.fillTab" :statusId="statusId"></mzCheckingDepart>
          </el-tab-pane>
        </el-tabs>
      </div>
    </div>
...
<script>
    import { getOtherCnt } from '@/api/kpi/onlineCheckDepart/reportingDepart'
    import reporting from './onlineCheckCom/reporting'
    import checking from './onlineCheckCom/checking'
    import mzCheckingDepart from '../components/onlineCheckCom/mzCheckingDepart'
    export default {
      name: 'otherCheckDepart',
      components: {
        reporting, checking, mzCheckingDepart
      },
      methods: {
        getOtherCnt,
        handleChange() {
          ...
          this.$refs.mzCheckingDepart.getxzList(this.statusId)
        }
      },
      data() {
        return {
