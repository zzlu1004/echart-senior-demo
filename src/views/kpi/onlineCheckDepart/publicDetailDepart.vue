      ...
      <div class="bg-gray p2 mb2 relative">
        ...
        <div>
          <el-row>
            ...
            <el-col :span="6">
              <span>结束时间 ：</span>
              <span>{{schema.finishedDate?schema.finishedDate.substring(0,10):''}}</span>
            </el-col>
          ...
          <el-col :span="8" class="bd p2 clearfix">
            <div class="half-width f-left bd-right center bold">
              ...
              <div class="color-title h24">
                {{schema.markCount}}/{{schema.departCount}}
              </div>
            </div>
          ...
          <el-col :span="16" class="pl4">
            <div class="bd p2 clearfix">
              <div v-for="(item,index) of schema.grades" :key="item.gradeId"
                  ...
                  >
                <div class="pb2">
                  {{item.gradeName}}档（ {{item.markCount}}个 ）
                </div>
                <div class="h24">
                  {{item.percent}}%
                </div>
          ...
          <fd-table
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <span class="color-blue pointer" @click="gotoMyResult(scope.row)">{{scope.row.departName}}</span>
              </template>
            </el-table-column>
            <el-table-column
              prop="departTypeName"
              ...
              <template slot-scope="scope">
                <div>
                  <span v-for="(key, index) of levelMap" :key="key.id">
                    <!--当index === 3，即最后一档无mr3样式 ；edit ? 'pointer ' : ''，即不可编辑时鼠标样式 ；通过key.id === scope.row.gradeId判断当前那一档高亮显示-->
...
<script>
  ...
  import { getInfoById, currentChange, sizeChange, gotoMyResult, searchByKeyword, downloadTable,
    ...
    sortByNum } from '@/api/kpi/onlineCheckDepart/publicDetailDepart'
  export default {
    name: '',
    components: {
    ...
    mounted() {
      ...
      // 点击“编辑”进入的page为“edit”，点击“方案名称”进入“detail”
