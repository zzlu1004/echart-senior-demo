    ...
    <div class="container-inner bg-white" :style="{'min-height': $store.getters.globals.height - 100 + 'px'}">
        <div class="mb2 clearfix">
          <el-row>
            ...
            <el-col class="pl2" :span="8">
              ...
              <div class="except-w4-full">
                <el-select size="small" v-model="params.schemaId" placeholder="请选择">
                  <el-option
        ...
        <div v-if="!isnull">
          <div class="bd clearfix p3 full-div relative">
            <el-row class="mb2">
              <el-col :span="8">
                <div class="f-left color-title">人员类型：</div>
                <div class="hidden text-indent0">{{userInfo.staffTypeName || '--'}}</div>
              </el-col>
              <el-col class="relative" :span="8">
                <div class="f-left color-title">部门：</div>
                <div class="hidden text-indent0">{{userInfo.departName || '--'}}</div>
              </el-col>
              <el-col class="relative" :span="8">
                <div class="f-left color-title">部门正副职：</div>
                <div class="hidden text-indent0">{{userInfo.chairName || '--'}}</div>
              </el-col>
            </el-row>
            <el-row class="mb2">
              <el-col class="relative" :span="8">
            ...
            <el-row>
              <el-col class="relative" :span="8">
                ...
                <div class="hidden text-indent0">{{schema.schemaFinishedTime?schema.schemaFinishedTime:'--'}}</div>
              </el-col>
            </el-row>
            <span class="absolute name-box h32 color-title bold">
              {{userInfo.staffName}}
            </span>
...
<script>
  import { getSchemas, getTargetList, downloadMyResult, refreshAll, changeType, emitParentTypeByMyresult } from '@/api/kpi/checkResult/myResult'
  ...
  export default {
    ...
    methods: {
      getSchemas, getTargetList, downloadMyResult, refreshAll, changeType, emitParentTypeByMyresult
    },
    watch: {
      indexData(d) {
