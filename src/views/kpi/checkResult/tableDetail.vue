    ...
    <div class="container-inner bg-white" :style="{'min-height': height - 100 + 'px'}">
      <div class="bd-dashed-bottom mb2">
        <div class="mb2">
          ...
          <el-row>
            <el-col :span="8">
              ...
              <div class="except-w4-full">
                <el-select
                    v-model="peopleParams.staffTypeIds"
                    multiple
                    size="small"
                    filterable
                    clearable
          ...
          <el-row class="mt2" v-if="isOpen">
            <el-col :span="8">
              <div class="absolute pr1 py1">法律职务：</div>
              <div class="except-w4-full">
                <el-select
                v-model="peopleParams.legalPositionIds"
                placeholder="请选择"
                size="small"
                multiple
                filterable
            ...
            <el-col class="pl2" :span="8">
              ...
              <div class="except-w4-full">
                <el-select
                v-model="peopleParams.judgeLevelIds"
                placeholder="请选择"
                size="small"
                multiple
                filterable
            ...
            <el-col class="pl2" :span="8">
              <div class="absolute pr1 py1">进入途径：</div>
              <div class="except-w4-full">
                <el-select
                v-model="peopleParams.resourceIds"
                placeholder="请选择"
                multiple
                size="small"
                filterable
        ...
        <div class="pb2 right-align">
          <el-button v-if="!isOpen" size="small" @click="isOpen = true">展开</el-button>
          <el-button v-else size="small" @click="isOpen = false">收起</el-button>
      ...
      <div>
            ...
            <el-col :span="grades.length>0?12:24">
              <el-row class="bd py2" >
                ...
                <el-col class="bd-right t-center bold" :span="6">
                  <div class="color-title pb2">考核人数</div>
            ...
            <el-col :span="grades.length>0?12:0">
              <el-row class="bd py2">
                <el-col v-for="(item, index) of grades"
                  ...
                  <div class="pb2">{{item.name}}档({{item.value}}人)</div>
        ...
        <div class="tab-box">
          <el-tabs v-model="category" type="card">
                    ...
                    <el-table
                      ...
                      >
                        <template slot-scope="scope">
                          <!-- 姓名可反查 -->
                          <span v-if="info.field === 'staffName'" class="color-blue pointer" @click="staffNamePeg(scope.row)">
                      {{scope.row.staffName}}</span>
                          <span v-else-if="info.field === 'departName'||info.field === 'rank'||info.field === 'chairName' || info.field === 'gradeName'">
                  ...
                  <div class="relative print-title">
                    ...
                    <el-table
                      ...
                      >
                        <template slot-scope="scope">
                          <!-- 姓名可反查 -->
                          <span v-if="info.field === 'staffName'">
                      {{scope.row.staffName}}</span>
                          <span v-else-if="info.field === 'departName'||info.field === 'rank'||info.field === 'chairName'  || info.field === 'gradeName'">
                      {{scope.row[info.field] ? scope.row[info.field]:'--'}}</span>
                          <span v-else>{{fd.data.saveFloat(scope.row[info.field], 2)}}</span>
            ...
            <el-tab-pane label="指标均值" name="biz" v-if="elements.average">
              <div v-if="category ==='biz'">
                <div class="clearfix pb2 table-search">
                  <span class="f-left">
                    <el-popover
                      placement="right"
                    ...
                    >
                      <div class="scroll-body">
                        <el-checkbox-group v-model="averageCheckedCols" @change="changeAverageCols">
                          <div class="pb1" v-for="cols of averageCheckbox" :key="cols.field">
                            <el-checkbox :label="cols.field">{{cols.title}}</el-checkbox>
                          </div>
                        </el-checkbox-group>
                      </div>
                  ...
                  <span class="f-right mr3">
                    <fd-search v-if="category === 'biz'" @search="searchByKeyword"
                               v-model="detailVal"></fd-search>
                  </span>
                </div>
                <div id="tableAveragePanel">
                  <div id="tableAverage">
                    <div class="relative print-title">
                      <div class="bold h16 title-hide absolute print-name center" >{{printTitle}}</div>
                      <el-table
                        v-loading="tableAverageLoading"
                        element-loading-text="数据正在加载中"
                  ...
                  <div id="tableAverageDisply" v-if="tableDisplay">
                    <div class="relative print-title">
                      <div class="bold h16 title-hide absolute print-name center" >{{printTitle}}</div>
                      <el-table
                        :data="averageTableData2"
                        border
                      >
                        <multi-head
                          v-for="item of treeAverage"
                  ...
                  <fd-pagination
                    :limit="averageParams.limit"
                    :total="averageTotal"
                    :offset="averageParams.offset"
                    sizes="sizes"
                    :pageArr="[10,20,30,40,50]"
                    :currentChange="currentChange"
                    :sizeChange="sizeChange">
...
<script>
  ...
  import { courtNodeClick, getTotalResult, getTotalResult2, getDetailResult, getDetailResult2, getAverageResult,
    searchByKeyword, changeTotalCols, changeDetailCols, changeCols, downloadTableDetail, getSchemas, refreshAll,
    sortByNum, getJudgeLevel, getAlignStyle, printTable, printTableTitle, staffNamePeg, clickBtn, getDepartData,
    currentChange, sizeChange, mzIndexPeg, changeAverageCols, getAverageResult2 } from '@/api/kpi/checkResult/tableDetail'
  export default {
    name: 'tableDetail',
    components: {
    ...
    data() {
      return {
        ...
        isChangeAverageCols: false, // 是否点击了自定义表格
        changeHeader: [],
        changeDetailHeader: [],
        changeAverageHeader: [], // 指标均值自定义表格更改后的表格头部
        changeCheck: [],
        changeDetailCheck: [],
        changeAverageCheck: [], // 指标均值表格中更改自定义表格后的v-model数据
        totalCheckedCols: [],
        totalCheckbox: [],
        totalCheckboxName: [],
        detailCheckedCols: [],
        detailCheckbox: [],
        detailCheckboxName: [],
        averageCheckbox: [], // 自定义列表数组中的数据
        averageCheckedCols: [], // 选中的自定义列表数组中的数据
        tableTotalLoading: false,
        tableDetailLoading: false,
        tableAverageLoading: false,
        category: 'total',
        ...
        averColumns: ['approvalScore', 'avgApprovalScore'],
        cols: [],
        colObj: {
          approvalScore: '指标值',
          formulaScore: '指标初始分',
    ...
    methods: {
      ...
      changeTotalCols, changeDetailCols, changeCols, downloadTableDetail, sortByNum, printTable, printTableTitle, getJudgeLevel,
      staffNamePeg, clickBtn, currentChange, sizeChange, getAlignStyle, mzIndexPeg, getAverageResult, changeAverageCols, getAverageResult2,
      getDataField(data) {
        return data.id === 'departName' || data.id === 'staffName' || data.id === 'chairName' || data.id === 'allScore' ||
          data.id === 'rank' || data.id === 'gradeName'
      },
      updateSchema() {
        this.fd.req({
          method: 'put',
          url: 'api/schemas/refresh/result?schemaId=' + this.baseParams.schemaId
        }).then(response => {
          this.$message({
            type: 'info',
...
.table-detail{
  ...
  .tab-box .el-tabs__item.is-active{
    color:#1b95e7;
  }
}
</style>
