...
<script>
  ...
  import { searchByKeyword, currentChange, sizeChange, getActiveSchemas, getCompletes, getTodos,
    downloadTable, sortByNum, sortByNum2, currentChange2, sizeChange2, searchByKeyword2, getAllTables } from '@/api/kpi/onlineCheck/checkProgress'
  export default {
    name: '',
    data() {
      const fields = [
      ...
      const completeFields = fields.concat([{
        field: 'progressRat',
        title: '考核进度',
        sortable: 'custom',
        formatter: (row, value) => {
      ...
      const todoFields = fields.concat([{
        field: 'progressRat',
        title: '待办进度',
        sortable: 'custom',
        formatter: (row, value) => {
    ...
    computed: {
      elements() {
        return this.$store.getters.elements.checkProgress || {}
      }
    },
    watch: {
    }
