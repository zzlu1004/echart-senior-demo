...
<script>import fdTable from 'fd/table'
  ...
  import { handleSelect, countPeg, currentChange, sizeChange, searchByKeyword, downloadTable, refreshCondition,
    refreshPunishAll, getMonthTotal, getYearTotal, getColumns, sortByNum } from '@/api/kpi/newRecord/studyDevelop'
  export default {
    name: 'workReward',
    components: {
    ...
    computed: {
      elements() {
        return this.$store.getters.elements.workReward || {}
      }
    },
    mounted() {
      this.refreshPunishAll()
    },
    methods: {
      handleSelect, countPeg, currentChange, sizeChange, searchByKeyword, downloadTable, refreshPunishAll, getMonthTotal,
