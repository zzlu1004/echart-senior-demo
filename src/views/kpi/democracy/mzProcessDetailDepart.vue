              ...
              <div>
                ...
                >
                  <span class="inline-block height-normal line-height-normal pl2 pr5 bd tag-box relative mb1" :class="page!=='detail'? 'pointer':''"
                    ...
                    <!--<span v-else-if="item.staffType === 'k10018-03'||item.staffType === '4'">{{item[staffTypeMap[item.staffType]]}}</span>-->
                    <!--<span v-if="item.staffType === 'k10018-02'||item.staffType === 'k10018-04'||item.staffType === 'k10018-05'">{{item.approvalType === 'k10029-01' ? '(会签)' : '(或签)'}}</span>-->
...
<script>
  ...
  import { getFlowById, cancelOperate, sureOperate, roleSelect, editAuditor,
    ...
    showApproveDes, showFillDes, getRoles } from '@/api/kpi/democracy/mzProcessDetailDepart'
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        setRules: {
          nodes: [
            { required: true, message: '请选择测评人', trigger: 'change' }
          ]
        },
        peopleForm: {
          staffType: '',
