  ...
  <div :class="isComponent?'':'app-container'">
    <div class="bg-white"
      ...
      <div class="relative">
        ...
        <div class="bd-dashed-left personal-msg auto" :style="{'height': $store.getters.globals.height - 150 + 'px', 'margin-left': '180px' }">
          <!--基本信息-->
          <div ref="basic" v-if="elements.basic && userInfo.id">
            <fd-personal-basic :userMsg="userInfo" :edit=false v-bind:hasActive="hasActive"></fd-personal-basic>
          </div>
          <!--简历信息-->
          <div class="mt3" ref="resume" v-if="elements.resume && userInfo.id">
            <fd-personal-resume v-bind:userInfo="userInfo" :edit=false v-bind:hasActive="hasActive"></fd-personal-resume>
          </div>
          <!--教育信息-->
          <div class="mt3" ref="education" v-if="elements.education && userInfo.id">
            <fd-personal-education v-bind:userInfo="userInfo" :edit=false v-bind:hasActive="hasActive"></fd-personal-education>
          </div>
          <!--家庭信息-->
          <div class="mt3" ref="family" v-if="elements.family && userInfo.id">
            <fd-personal-family v-bind:userInfo="userInfo" :edit=false v-bind:hasActive="hasActive"></fd-personal-family>
          </div>
          <!--法律职务-->
          <div class="mt3" ref="lawDuty" v-if="elements.lawDuty && userInfo.id">
            <fd-personal-duty v-bind:userInfo="userInfo" :edit=false v-bind:hasActive="hasActive"></fd-personal-duty>
          </div>
          <!--部门变更-->
          <div class="mt3" ref="changeDepart" v-if="elements.change && userInfo.id">
            <fd-personal-depart v-bind:userInfo="userInfo" :edit=false v-bind:hasActive="hasActive"></fd-personal-depart>
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      elements() {
        return this.$store.getters.elements.personalMsg || {}
      },
      staffId() {
