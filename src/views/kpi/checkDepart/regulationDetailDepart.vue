    ...
    <div class="container-inner has-fixed-bottom bg-white" :style="{'min-height': $store.getters.globals.height - 100 + 'px'}">
      ...
      <el-form class="py2"
        ...
        <el-form-item label="指标体系">
          <div>
            <!-- <fd-menu-table v-bind:menus="menus"
        ...
        <!--<el-form-item label="指标计算规则" prop="ruleType">-->
          <!--<el-radio disabled v-for="item of ruleTypes" v-model="regulation.ruleType" :key = "item.id" :label="item.id">{{item.text}}</el-radio>-->
        <!--</el-form-item>-->
        <!--<el-form-item label="" prop="formula" v-show="regulation.ruleType === 'k10030-02'">-->
          <!--<el-input readonly v-model="regulation.formula"></el-input>-->
          <!--<fd-formula-des v-if="regulation.ruleType !== 'k10030-02'" :des="des" :example="[]" ></fd-formula-des>-->
        <!--</el-form-item>-->
...
<script>
  ...
  import RegulationTableDepart from '../components/regulationTableDepart'
  export default {
    name: 'regulationDetailDepart',
    components: { RegulationTableDepart, FdTable, regulationTableDepart, FdFormulaDes },
    props: {},
    data() {
      return {
        target: {
