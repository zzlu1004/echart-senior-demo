export function addRegulation() {
  this.fd.tools.goDetail.call(this, 'addRegulation', { query: { id: '' }})
}
...
export function rowClick(row, event, column) {
  if (column.property === 'name') {
    this.fd.tools.goDetail.call(this, 'regulationDetail', { query: { id: row.id }})
  }
}
// 关联方案列表
...
export function editRegulation(row) {
  if (row.status !== this.forbid) {
    // this.$router.push({ name: 'addRegulation', query: { id: row.id, schemaId: this.schemaId }})
    this.fd.tools.goDetail.call(this, 'addRegulation', { query: { id: row.id, schemaId: this.schemaId }})
  }
}
// 复制细则
...
export function getRegulationIndexs() {
  this.fd.req({
    type: 'get',
    url: 'api/blocks/' + this.id + '/indexes2',
    data: {
      schemaId: this.schemaId
    }
  }).then(response => {
...
export function getRegulationSchemas() {
  ...
  this.fd.req({
    type: 'get',
    url: 'api/blocks/' + this.id + '/schemas',
    data: {
      schemaId: this.schemaId
    }
  }).then(response => {
