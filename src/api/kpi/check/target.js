...
export function addTarget(item) {
  ...
  if (this.params.indexType === 'newSystem') {
    this.fd.tools.goDetail.call(this, 'indicatorSetting', { query: this.targetForm })
  } else {
    this.targetVsible = true
    this.$nextTick(() => {
      this.targetForm = {
...
export function seeTarget(row1, column, cell, event) {
  ...
  if (column.property === 'name' && row.isLeaf) {
    ...
    this.targetForm = {
      ...
      departAvgNumeratorExpression: row.departAvgNumeratorExpression,
      departAvgNumeratorScope: row.departAvgNumeratorScope,
      departAvgDenominatorExpression: row.departAvgDenominatorExpression,
      departAvgDenominatorScope: row.departAvgDenominatorScope,
      // 新增业务线规则
...
export function downloadTable(type) {
  this.params.indexType = type
  this.params.indexName = this[this.valObj[type]]
  const url = 'api/indexes/download'
