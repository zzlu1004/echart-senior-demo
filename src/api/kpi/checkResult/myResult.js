export function getSchemas() {
  ...
  this.fd.req({
    type: 'get',
    url: 'api/schemas/publiced/current-user',
    data: {
...
export function getTargetList() {
  ...
  this.fd.req({
    type: 'get',
    url: 'api/assessmet/result/' + this.staffId,
    data: this.params
  }).then(response => {
    const data = response.data || {}
    ...
    this.fileName = (this.userInfo.staffName ? this.userInfo.staffName : '') + (this.schema.schemaName ? this.schema.schemaName : '') + this.typesMap[this.type] + '考核结果'
...
export function downloadMyResult() {
  ...
  // document.getElementById('myResult').removeChild(document.getElementsByClassName('el-popover')[0])
...
export function showPeg(row) {
  ...
  this.$nextTick(() => {
    this.showEdit = true
    this.getPegValue({ staffId: this.staffId }, row)
  })
}
