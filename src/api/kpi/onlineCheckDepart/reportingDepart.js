...
export function seeReportDetail(row) {
  ...
  this.fd.tools.goDetail.call(this, 'reportListDepart', { query: {
    index: row.index, departId: row.departId, blockId: row.blockId,
...
export function seeCheckDetail(row) {
  ...
  localStorage.setItem('obj', JSON.stringify(obj))
  this.fd.tools.goDetail.call(this, 'checkListDepart', { query: {
    index: row.index, departId: row.departId,
    blockId: row.blockId, schemaBlockType: row.schemaBlockType,
    schemaId: row.schemaId }})
}
...
export function seeRuleDetail(row, pRow) {
  ...
  localStorage.setItem('obj', JSON.stringify(obj))
  this.fd.tools.goDetail.call(this, 'ruleReportListDepart', { query: {
...
export function seeCheckRuleDetail(row, pRow) {
  ...
  localStorage.setItem('obj', JSON.stringify(obj))
  this.fd.tools.goDetail.call(this, 'ruleCheckListDepart', { query: {
    index: row.offset, blockId: row.blockId,
    schemaBlockType: row.schemaBlockType, schemaId: pRow.schemaId }})
}
// 其他考核--待填报的细则填报
...
export function getxzList(statusId = 1) {
  this.fd.req({
    url: 'api/dept/assessments/blocks',
    method: 'GET',
    data: { keyword: this.params.keyword, type: this.checkType, statusId }
  }).then((d) => {
    const data = d.data || []
    for (const item of data) {
      if (item.synthesises) {
...
export function getOtherCnt() {
  this.fd.req({
    url: 'api/dept/assessments/counts',
    method: 'GET',
