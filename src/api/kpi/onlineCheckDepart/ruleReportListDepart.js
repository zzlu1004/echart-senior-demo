...
export function getTragetData(type, data) {
  ...
  this.fd.req({
    url: 'api/dept/evaluations/current-user/' + type + '?actionType=fill',
    method: 'put',
    data: data
  }).then((d) => {
    this.$message({
...
export function seeCheckDetail(schema, row) {
  ...
  localStorage.setItem('obj', JSON.stringify(obj))
  this.$router.push({ name: 'otherCheckDepart-reportListDepart', query: {
    index: row.index, departId: row.departId, blockId: schema.blockId,
