...
export function getTargetByUrlId() {
  ...
  const url = 'api/dept/assessments/users/' + this.departId
...
export function getTargetById(url, paramsData) {
  ...
  this.fd.req({
    ...
    for (const item of data.indexes) {
      ...
      // 审核值
      if (item.approveScore === 0) {
        item.approveScore = item.approveScore + ''
      }
      // 前一流程的分数
      if (item.preScore === 0) {
...
export function getTragetData(targetsMsg, isRed) {
  ...
  this.fd.req({
    url: 'api/dept/evaluations/current-user/' + this.typeName + '?actionType=approve',
    method: 'put',
    data: data
  }).then((d) => {
    this.$message({
...
export function recallTarget(row) {
  this.$confirm('撤回后该指标需要重新审核提交，请问确定撤回《' + row.menuName + '》指标吗?', '撤回指标', {
    ...
    this.fd.req({
      url: 'api/dept/evaluations/current-user/flows/' + row.preNodeId + '/withdraw',
      method: 'put'
    }).then((d) => {
      this.$message({
        type: 'success',
