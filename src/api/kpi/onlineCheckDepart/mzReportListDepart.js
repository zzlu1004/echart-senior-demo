...
export function getRuleDetailById() {
  ...
  this.fd.req({
    url: 'api/dept/assessments/democratic/blocks/' + this.id,
    method: 'GET',
    data: this.params
  }).then((d) => {
    const data = d.data || {}
...
export function getTragetData(type) {
  ...
  this.fd.req({
    url: 'api/dept/evaluations/democratic/current-user/' + type,
    method: 'put',
...
export function withdrawAll(val) {
  this.fd.req({
    url: 'api/dept/evaluations/democratic/current-user/withdraw/all?schemaId=' + this.params.schemaId + '&blockId=' + this.params.blockId + '&schemaBlockType=' + this.params.schemaBlockType,
    method: 'PUT'
  }).then((d) => {
    this.$message({
      type: 'success',
