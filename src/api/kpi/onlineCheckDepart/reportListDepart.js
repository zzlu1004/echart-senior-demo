...
export function getPeopleListToTab() {
  this.fd.req({
    url: 'api/dept/assessments/users',
    method: 'GET',
    data: this.peopleParams
  }).then((d) => {
    const data = d.data || {}
    this.peopleData = data.data[0] || []
...
export function getPeopleList() {
  this.fd.req({
    ...
    // 点击上一个，下一个，需要重新从人员列表中获取departId
    this.getTargetByUrlId()
  })
}
// 通过id 获取方案卡片 以及 表格中待填报的数据
export function getTargetByUrlId() {
  ...
  if (this.resource === 'other') {
    ...
    url = 'api/dept/assessments/users/' + this.departId
  } else {
    // 我的考核--待填报的接口
    paramsData = this.fd.tools.merge(true, {}, this.params)
    url = 'api/dept/evaluations/current-user/details'
  }
...
export function getTargetById(url, paramsData) {
  ...
  this.fd.req({
    ...
    this.departName = this.detailRule.departName
...
export function seeRegularDetail(row, resource) {
  this.fd.tools.goDetail.call(this, 'checkDetailDepart', { query: { id: row.schemaFlowId, resource: resource }})
}
// 校验填报+审核分数值
...
export function getTragetData(targetsMsg, isRed) {
  ...
  for (const key in targetsMsg) {
    ...
    if (targetsMsg[key].fillEditable) {
      data.push({
        ...
        departId: this.detailRule.departId,
        departName: this.detailRule.departName,
  ...
  this.fd.req({
    url: 'api/dept/evaluations/current-user/' + this.typeName + '?actionType=fill',
    method: 'put',
    data: data
  }).then((d) => {
    this.$message({
...
export function sureAddAttach() {
  this.$refs.attachForm.validate((valid) => {
    if (valid) {
      ...
      this.fd.req({
        url: 'api/dept/evaluations/current-user/' + this.currentItem.fillNodeActionId + '/content',
        method: 'put',
...
export function recallTarget(row) {
  this.$confirm('撤回后该指标需要重新填报提交，请问确定撤回《' + row.menuName + '》指标吗?', '撤回指标', {
    ...
    this.fd.req({
      url: 'api/dept/evaluations/current-user/flows/' + row.fillNodeId + '/withdraw',
      method: 'put'
    }).then((d) => {
      this.$message({
        type: 'success',
...
export function revocationAll() {
  this.$confirm('全部撤回后所有可撤回的指标需要重新填报提交,请问确定撤回全部的指标吗?', '全部撤回', {
    ...
    this.fd.req({
      url: 'api/dept/evaluations/current-user/flows/users/withdraw',
      method: 'put',
      data: {
