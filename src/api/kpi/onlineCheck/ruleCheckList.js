...
export function refreshTableData(data) {
  ...
  this.$nextTick(() => {
    this.tree = data.menus.filter((item, index) => {
      return item.menuLevel === null
    })
...
export function getTragetData(type, isRed) {
  ...
  this.fd.req({
    url: 'api/evaluations/current-user/' + type + '?actionType=approve',
    method: 'put',
    data: data
  }).then((d) => {
    this.$message({
...
export function seeCheckDetail(schema, row) {
  ...
  localStorage.setItem('obj', JSON.stringify(obj))
  this.$router.push({ name: 'otherCheck-checkList', query: {
...
export function sortByNum(column, prop, order) {
  ...
  // 当未审核未保存草稿时查看页面点击排序时不可排序,使用this.edit和this.isFlag来区分
  for (const item of this.tableData) {
    for (const info of item.indexes) {
      if (info.draftScore) {
        this.isFlag = true
  ...
  if (this.edit) {
    for (const item of this.tableData) {
      for (const info of item.indexes) {
        if (item['approveScore' + info.indexId] === 'null') {
          info.approveScore = ' '
        } else {
          info.approveScore = item['approveScore' + info.indexId]
        }
        if (item['approveScore' + info.indexId] === 'null') {
          info.draftScore = ' '
        } else {
          info.draftScore = item['approveScore' + info.indexId]
        }
        data.push(info)
      }
    }
  ...
  this.indexMenuId = column.prop
  this.fd.req({
    url: 'api/assessments/blocks/sort?orderField=' + column.prop + '&orderType=' + column.order,
    method: 'post',
    data: data
  }).then((d) => {
    const staffs = d.data
    for (const item of staffs) {
      ...
      for (const info of item.indexes) {
        ...
        if (info.preScore === null) {
          info.preScore = ''
        }
        if (info.fillScore === null) {
          info.fillScore = ''
        }
        if (info.draftScore === null) {
          info.draftScore = ''
        }
        item['approveScore' + info.indexId] = (info.approveScore + '') || (info.preScore + '')
        if (!fillEditable) {
