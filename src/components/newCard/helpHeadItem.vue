<template>
  <div :class="'headItem '+(option.disable != undefined && !data[option.disable])"
       @click.stop="eventEmit(option.event, data)"
       v-show="option.show === undefined && !data[option.show]">
    <div :style="{backgroundImage: 'url(' + option.map[data[option.field]] + ')' }"
         v-if="option.type === 'img'"></div>

    <span :class="'' + option.class"
          v-else-if="option.type === 'checkbox'"></span>

    <i :class="'at pointer ' + collapse + ' ' + option.class"
       @click.stop="changeCollapse"
       v-else-if="option.type === 'collapse'"></i>

    <div v-else-if="option.type === 'html'">
      <span :class="'f-left dot-hidden ' + option.class"
            :style="{'width': option.width}"
            v-html="option.map[data[option.field]]"
            v-if="option.tooltip === undefined && !data[option.tooltip]"></span>
      <!--添加tooltip提示-->
      <fd-tooltip
    ...
    <div v-else-if="option.type === 'text'">
      <span :class="'f-left dot-hidden ' + option.class"
            :style="{'width': option.width}"
            v-if="option.tooltip === undefined && !data[option.tooltip]">{{ data[option.field] }}</span>
      <!--添加tooltip提示-->
      <fd-tooltip
        :content="this.fd.tools.type(option.tooltip) === 'String' ? option.tooltip : option.tooltip[data[option.field]]"
        :placement="option.placement"
    ...
    <span :class="'f-left dot-hidden ' + option.class"
          :style="{'width': option.width}"
          v-else-if="option.type === 'fixedText'"
          v-text="option.text"></span>

    <span :class="'f-left dot-hidden ' + option.class"
          :style="{'width': option.width}"
          v-else-if="option.type === 'time'">{{ data[option.field] | parseTime(option.format) }}</span>
    ...
    <div v-else-if="option.type === 'button'">
      <span :class="((item.class || 'mx1') + ((item.disable === undefined || data[item.disable])?' pointer color-link': ' color-forbid'))"
            :key="item.type"
            @click.stop="eventEmit(item.event, data)"
            v-for="item of buttons">{{item.text}}</span>
      <el-dropdown
        ...
        <el-dropdown-menu slot="dropdown">
          <el-dropdown-item :command="{event: it.event, data: data}"
                            :key="it.type"
                            v-for="it of moreButtons">
